...
test('GET /health', async () => {
  ...
  assert.equal(res.body.service, 'quotes-service');
});

test('GET / returns quotes', async () => {
...
test('GET /:id resolves or 404s appropriately', async () => {
  ...
  const notFound = await request(app).get('/999');
